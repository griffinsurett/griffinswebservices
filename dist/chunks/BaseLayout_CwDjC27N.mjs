...
import 'piccolore';
import 'clsx';
import { s as siteData } from './speakers_ClnnXv1O.mjs';
/* empty css                         */
/* empty css                          */
/* empty css                              */
/* empty css                                 */
import { b as shouldShowCollectionCTA, $ as $$SectionHeader, t as toArray, B as Button, d as $$ContentBridge, A as Accordion, g as generateVideoPoster, V as Video, H as Heading, _ as __vite_glob_0_26, l as logoImg } from './accordion_T_3z6Val.mjs';
import { jsx, jsxs, Fragment as Fragment$1 } from 'react/jsx-runtime';
import { F as FeatureCard } from './feature-cards_B0lNKt4Q.mjs';
...
import { u as useVisibility, a as useScrollInteraction } from './visibility-hooks_h8bFi65s.mjs';
/* empty css                                                                      */
import { isRemoteAllowed } from '@astrojs/internal-helpers/remote';
import * as mime from 'mrmime';
/* empty css                                                                   */
import { I as IconListItem, a as Icon, A as AnimatedBorder } from './ui-primitives_BW0hwiYo.mjs';
/* empty css                                                                             */
import { P as PortfolioScreenShowcase, b as PortfolioCarousel, T as TechStackSection, _ as __vite_glob_0_23 } from './carousels_DzdpIwnp.mjs';
/* empty css                                                                              */
/* empty css                                                               */
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content_BLGvmwIH.mjs');
...
function createGetCollection({
  ...
  return async function getCollection(collection, filter) {
    ...
    if (collection in contentCollectionToEntryMap) {
      ...
      const { default: imageAssetMap } = await import('./content-assets_i2m4FaP0.mjs');
      const result = [];
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } = await import('./content-modules_d6U4zoWj.mjs');
...
async function buildRelationshipGraph(options = {}) {
  ...
  const { extractRelationConfig, normalizeReference, isParentField } = await import('./schema_DP9S4hz_.mjs');
...
const $$IconListVariant = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<section${addAttribute(id, "id")}${addAttribute(`py-16 ${className}`.trim(), "class")}> <div class="mx-auto inner-section"> <div${addAttribute(`flex flex-col lg:space-x-15 ${showHeader ? "lg:flex-row lg:items-start" : ""}`.trim(), "class")}> ${showHeader && renderTemplate`<div class="w-full lg:w-1/2 sticky-section lg:self-start"> ${renderComponent($$result, "SectionHeader", $$SectionHeader, { "title": sectionTitle, "heading": heading, "description": description, "className": "space-y-4 text-left w-full", "headingTag": headingTag, "headingClassName": headingClassName, "descriptionClassName": "large-text leading-relaxed", "titleVisibleRootMargin": 0 })} ${Astro2.slots.has("under-text") && renderTemplate`<div class="mt-8"> ${renderSlot($$result, $$slots["under-text"])} </div>`} </div>`} ${safeItems.length > 0 && renderTemplate`<div${addAttribute(`w-full py-6 lg:py-0 ${showHeader ? "lg:w-1/2" : ""}`.trim(), "class")}> <ul${addAttribute(`flex flex-col gap-4 list-none`.trim(), "class")}> ${safeItems.map((item) => {
...
const $$QuickFactsVariant = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${safeItems.length > 0 && renderTemplate`${maybeRenderHead()}<div${addAttribute(id, "id")}${addAttribute(combinedClass, "class")}>${safeItems.map((item) => {
    const record = item && typeof item === "object" && "data" in item ? item.data ?? item : item ?? {};
    return renderTemplate`<div class="p-2 rounded-2xl card-bg"><p class="text-xs uppercase tracking-[0.2em] text-muted">${record.title ?? record.label}</p><p class="text-heading font-semibold">${record.description ?? record.value}</p></div>`;
  })}</div>`}`;
...
const CookiePreferencesModal$1 = lazy(() => import('./CookiePreferencesModal_klQKKdEi.mjs'));
...
const CookiePreferencesModal = lazy(() => import('./CookiePreferencesModal_klQKKdEi.mjs'));
...
function AccessibilityButton() {
  const { Component: Modal, isOpen, close } = useLazyLoad(
    () => import('./AccessibilityModal_DuFjrJw_.mjs'),

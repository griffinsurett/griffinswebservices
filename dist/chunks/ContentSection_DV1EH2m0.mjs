import { a as createComponent, c as createAstro, m as maybeRenderHead, b as addAttribute, r as renderSlot, e as renderTemplate, d as renderComponent } from './astro/server_BujBp8IR.mjs';
import 'piccolore';
import { $ as $$SectionHeader, B as Button } from './accordion_CNWWAjUA.mjs';
import { I as IconListItem } from './ui-primitives_qUXBbRTf.mjs';
import 'clsx';
...
const $$ContentSection = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<section id="about" class="outer-section relative"> <div class="inner-section space-y-20"> <div class="flex flex-col gap-12 lg:flex-row lg:items-start"> <article class="space-y-8 lg:flex-[1.25] min-w-0 sticky-section"> ${renderComponent($$result, "SectionHeader", $$SectionHeader, { "title": headingTitle, "headingBefore": headingBefore, "headingEmphasis": headingEmphasis, "headingAfter": headingAfter, "className": "text-left", "headingClassName": "h2 mb-6", "description": headingDescription, "descriptionClassName": "text-lg lg:text-xl text-text max-w-3xl" })} ${renderSlot($$result, $$slots["text"])} ${renderSlot($$result, $$slots["under-text"])} <div class="flex flex-wrap gap-4"> ${renderComponent($$result, "Button", Button, { "variant": "primary", "size": "lg", "href": primaryCTA.href, "rightIcon": primaryCTA.rightIcon }, { "default": ($$result2) => renderTemplate`${primaryCTA.text}` })} ${secondaryCTA && renderTemplate`${renderComponent($$result, "Button", Button, { "client:visible": true, "variant": "secondary", "size": "lg", "href": secondaryCTA.href, "rightIcon": secondaryCTA.rightIcon, "client:component-hydration": "visible", "client:component-path": "@/components/Button/Button", "client:component-export": "default" }, { "default": ($$result2) => renderTemplate`${secondaryCTA.text}` })}`} </div> </article> <aside class="sticky-section lg:flex-[0.85] w-full"> <div class="rounded-3xl card-bg p-4 lg:p-8 shadow-2xl space-y-6"> ${renderComponent($$result, "IconListItem", IconListItem, { "client:visible": true, "data": {
    icon: sidebarIcon,
    title: sidebarTitle,
    description: sidebarEyebrow
  }, "layout": "horizontal", "alignment": "left", "className": "items-start gap-4 w-full", "iconSize": "lg", "titleClassName": "h3", "descriptionClassName": "text-xs font-semibold uppercase tracking-[0.3em] emphasized-text", "containerClassName": "space-y-1", "client:component-hydration": "visible", "client:component-path": "@/components/LoopComponents/IconListItem", "client:component-export": "default" })} <p class="text-muted leading-relaxed mt-4"> ${sidebarDescription} </p> ${sidebarFeatures.length > 0 && renderTemplate`<ul class="mt-6 space-y-3"> ${sidebarFeatures.map((feature, index) => renderTemplate`<li class="flex items-start gap-3"${addAttribute(sidebarFeatureKey(feature, index), "key")}> <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-accent"> <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"> <path d="M5 12l4 4L19 6" stroke-linecap="round" stroke-linejoin="round"></path> </svg> </span> <p class="text-muted text-sm leading-relaxed"> ${sidebarFeatureText(feature)} </p> </li>`)} </ul>`} <div class="mt-8 space-y-6"> ${renderSlot($$result, $$slots["quick-facts"])} </div> ${renderComponent($$result, "ContentContainer", $$ContentContainer, { "icon": sidebarFootIcon, "title": sidebarFootTitle, "description": sidebarFootDescription, "className": "mt-8" })} </div> </aside> </div> ${renderSlot($$result, $$slots["bottom-section"])} </div> </section>`;

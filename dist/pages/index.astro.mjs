...
import { q as query, a as $$ContentRenderer, $ as $$BaseLayout, o as roots, t as sortByOrder } from '../chunks/BaseLayout_BD8xUcJo.mjs';
import { s as siteData } from '../chunks/wordpress_CB8A8Itm.mjs';
import { $ as $$DoubleCard, a as $$CTASection } from '../chunks/DoubleCard_SJoqg0Ns.mjs';
import 'piccolore';
import { $ as $$SectionHeader, B as Button, a as animationProps } from '../chunks/accordion_D9CVjMOr.mjs';
import 'clsx';
/* empty css                                 */
...
const $$ScrollIndicator = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<a${addAttribute(href, "href")}${addAttribute(`scroll-indicator absolute bottom-8 sm:bottom-12 md:bottom-12 left-1/2 -translate-x-1/2 cursor-pointer no-underline z-20 ${className}`, "class")} aria-label="Scroll down for more content" data-astro-cid-7ivj2o3m> <div class="flex items-center justify-center p-1 border border-text rounded-full shadow-lg transition-all duration-300 ease-in-out" data-astro-cid-7ivj2o3m> <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-text transition-all duration-300 w-4 h-4 md:w-5 md:h-5 hover:translate-y-0.5" data-astro-cid-7ivj2o3m> <path d="M12 5v14M19 12l-7 7-7-7" data-astro-cid-7ivj2o3m></path> </svg> </div> </a> `;
...
const $$Index = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "BaseLayout", $$BaseLayout, { "title": "Home", "description": siteData.description }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<main class="flex-1"> <!-- <GradientLayout> --> <!-- Lightweight text-only hero - zero carousel JS on initial load --> <!-- <TextHero
  ...
  }, "description": "You deserve a lightning-fast, secure, and scalable website that turn your visitors into customers. Every site we build is built to dominate and stand the test of time.", "columns": 3, "gap": "lg", "showButtonSection": false, "className": "pt-35 lg:pt-20" })} <!-- Filtered Capabilities by Parent (auto-detects parent field) --> ${renderComponent($$result2, "ContentRenderer", $$ContentRenderer, { "query": query("capabilities").where((entry) => !!entry.data.parent).orderBy(sortByOrder()), "variant": "FilteredCardVariant", "id": "capabilities-filtered", "title": "Our Capabilities", "heading": {
    before: "Explore our",
    text: "specialized skills",
    after: "by category."
  }, "description": "Filter through our expertise areas to find exactly what you need for your project.", "filter": { allLabel: "All Services", showCount: true }, "columns": 3, "filterSize": "sm" })} <!-- Website Benefits --> ${renderComponent($$result2, "ContentRenderer", $$ContentRenderer, { "query": query("benefits").orderBy(sortByOrder()), "variant": "BenefitsVariant", "columns": 2, "gap": "lg", "title": "Why Choose Us", "heading": "What Makes Our Websites Different", ",": true }, { "default": ($$result3) => renderTemplate`   ` })} <!-- About Us Two-Column Section --> <!-- <ContentRenderer
    ...
    />
    </ContentRenderer> --> <!-- <ContentRenderer query={query("projects")} variant="PortfolioVariant" /> --> ${renderComponent($$result2, "ContentRenderer", $$ContentRenderer, { "query": query("solutions"), "variant": "IconListVariant", "heading": "Premium Websites for Any Business, Built for Your Growth.", ",": true, "title": "Website Services", "description": "We design top-tier websites for businesses in every industry \u2014 from local services to global brands. Every site is custom-built for your goals, lightning-fast, secure, and easy to manage. No templates, no bloat \u2014 just a powerful online presence that looks sharp, performs flawlessly, and grows with you.", "autoAdvanceDelay": 2e3 })} <!-- <ColorBorderLayout borderPosition="both"> --> ${renderComponent($$result2, "ContentRenderer", $$ContentRenderer, { "query": query("solutions"), "variant": "VideoAccordionVariant", "title": "Website Solutions", "heading": "Premium Websites for Any Business, Built for Your Growth.", "description": "Dive deeper into the speed, visuals, and UX of every solution with quick video overviews pulled straight from our library.", "className": "pt-16", "autoAdvanceDelay": 3500 })} <!-- </ColorBorderLayout> --> <!-- Testimonials with rating filter --> ${renderComponent($$result2, "ContentRenderer", $$ContentRenderer, { "query": query("testimonials").where((entry) => entry.data.rating >= 4), "variant": "TestimonialCarouselVariant", "id": "testimonials-home" })} <!-- Portfolio with sorting --> ${renderComponent($$result2, "ContentRenderer", $$ContentRenderer, { "query": query("projects").orderBy(sortByOrder("publishDate")), "variant": "PortfolioVariant" })} <!-- Technologies --> ${renderComponent($$result2, "ContentRenderer", $$ContentRenderer, { "query": query("technologies").orderBy(sortByOrder()), "variant": "TechnologiesVariant" })} <!-- Filtered query - still gets meta from authors --> <!-- <ContentRenderer
